...
export class Messages {
        ...
        voiceChannel
          ...
          .then(connection => {
            const dispatcher = connection.play(
              'http://www.pixtar.com.br/sons/OFolgado.mp3'
            );
          ...
          .catch(err => {
            this.message.reply(`eu não tenho permissão no canal <#${voiceChannel?.id}>`);
            console.log(err);
          });
    }
  }
  ...
  cmdStart () {
    ...
    this.message.reply(
      `Iniciando **${config.botname}** no canal <#${this.message.channel.id}>`
    );
  }
  cmdStop () {
    this.activeChannel = this.activeChannel.filter(
      channel => channel != (this.message.channel as any).name
    );
    this.message.reply(
      `Parando **${config.botname}** no canal <#${this.message.channel.id}>`
    );
  }
  cmdFolgado () {
      ...
      switch (command) {
        ...
        case 'rm':
          ...
          DB.client.query(
            DB.MESSAGE_REMOVE,
            [id, param],
            this.callbackRmMessage
          );
          break;
        default:
          var helpCommands = ['\n'];
          helpCommands.push(
            `Comandos **${config.botname}** - **${config.prefix}message**`
          );
          helpCommands.push(
            `**${config.prefix}message add <Menssage>** - Add Menssage in **${config.botname}**`
          );
          helpCommands.push(
            `**${config.prefix}message rm <ID or Menssagem>** - Remove Menssage in **${config.botname}**`
          );
  ...
  cmdHelp () {
    var helpCommands = ['\n'];
    helpCommands.push(
      `Comandos **${config.botname}** - **${config.prefix}help**`
    );
    helpCommands.push(
      `**${config.prefix}start** - Inicia o **${config.botname}**`
    );
    helpCommands.push(
      `**${config.prefix}stop** - Finaliza o **${config.botname}**`
    );
    helpCommands.push(
      `**${config.prefix}folgado** - **${config.botname}** join channel to voip "O Folgado" `
    );
    helpCommands.push(
      `**${config.prefix}messages** - Lista as Mensagens da **${config.botname}**`
    );
    helpCommands.push(
      `**${config.prefix}message add <Menssage>** - Add Menssage in **${config.botname}**`
    );
  ...
  callbackMessage = (err: any, result: any) => {
    if (err) {
      const msg =
        msgs.messages[Math.floor(Math.random() * msgs.messages.length)];
